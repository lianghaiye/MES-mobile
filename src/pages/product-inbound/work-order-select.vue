...
import { filterCompletedWorkOrders } from '@/mock/workOrderBridge'

const keyword = ref('')

const list = computed(() => filterCompletedWorkOrders({ keyword: keyword.value }))

function selectWorkOrder(item) {
  uni.navigateTo({
    url: `/pages/product-inbound/create-by-work-order?workOrderId=${item.id}`,
  })
}
</script>
