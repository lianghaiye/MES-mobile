  ...
  <view class="picker-mask" @tap="$emit('close')">
    <view class="picker-panel" @tap.stop>
      ...
      <scroll-view scroll-y class="picker-list">
        ...
        <view v-if="!filtered.length" class="empty">暂无匹配产品</view>
      </scroll-view>
    </view>
  </view>
</template>
...
import { searchSellableProducts } from '@/mock/productMaterialInfo'

const emit = defineEmits(['select', 'close'])

const keyword = ref('')

const filtered = computed(() => searchSellableProducts(keyword.value).slice(0, 50))
